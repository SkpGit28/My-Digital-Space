...
async function testShoulderFix() {
  ...
  // Isolate Sushant's portrait photo subject
  ...
  // To fix the right shoulder cut-off, let's extend the shirt background on the right edge
  // or apply a soft horizontal gradient mask to the right 30px so it smoothly fades out!
  for (let y = 0; y < height; y++) {
    for (let x = width - 40; x < width; x++) {
      const i = (y * width + x) * 4;
      const alpha = newBuf[i + 3];
      if (alpha > 0) {
        // Feather alpha linearly near the rightmost edge
        const distFromRight = (width - 1 - x); // 0 at rightmost edge, 39 at 40px in
        const fadeFactor = Math.min(1, Math.max(0, distFromRight / 35));
        newBuf[i + 3] = Math.round(alpha * fadeFactor);
      }
    }
  }
  ...
  // SVG Template
  // Photo Container right boundary is x=1130.
  // We place the subject at x=710, y=105, width=440, height=470.
  // With the feathering applied to the rightmost edge, the shirt's right edge softly dissolves into the light background with zero harsh cut-off line!
...
<svg width="1200" height="630" viewBox="0 0 1200 630" xmlns="http://www.w3.org/2000/svg">
  ...
  <g clip-path="url(#photoRightAlignedClip)">
    ...
    <!-- Sushant's Subject Photo positioned with top clearance & soft alpha feathered right edge to completely eliminate hard shoulder crop lines -->
    <image href="${subjectBase64}" x="710" y="105" width="430" height="465" preserveAspectRatio="xMidYMin meet" />
  </g>
...
`;
  ...
  console.log('Shoulder fix OG image generated successfully at:', outputPath);
}

testShoulderFix().catch(console.error);
